...
def solverRambusExperiments():
	...
	#volThresholdArray = [0.002]
	useLp = True

	
	timingFilename = "data/solver_time_rambus_det.csv"
...
def solverSchmittExperiments():
	...
	timingFilename = "solver_time_schmitt.csv"
	headers = ["Problem", "NumSolutions", "volRedThreshold"]
	for i in range(numTrials):
		headers.append("Run_"+str(i))
	csvTimeData = [headers]
	...
	for inputVoltage in inputVoltageArray:
		for lpThreshold in lpThresholdArray:
			...
			for n in range(numTrials):
				start = time.time()
				allHypers = schmittTrigger(inputVoltage, lpThreshold, numSolutions = "all", newtonHypers = True, useLp = useLp)
				end = time.time()
				numSolutions = len(allHypers)
				timeTaken = end - start
				print ("run ", n, "timeTaken", timeTaken)
...
def solverBenchmarkExperiments():
	...
	volRedThresholdArray = [0.3]

	useLp = True

	
	timingFilename = "solver_time_benchmark.csv"
	headers = ["Problem", "volRedThreshold"]
	for i in range(numTrials):
		headers.append("Run_"+str(i))
	csvTimeData = [headers]

	for problemType in problemTypeArray:
		for volRedThreshold in volRedThresholdArray:
			print ("problemType", problemType)
			timeData = ["benchmark_problem_"+problemType]

			for n in range(numTrials):
				start = time.time()
				singleVariableInequalities(problemType, volRedThreshold, numSolutions="all", newtonHypers=True, useLp=True)
				end = time.time()
				timeTaken = end - start
				print ("run ", n, "timeTaken", timeTaken)

				if n == 0:
					timeData.append(volRedThreshold)
...
def dRealRambusExperiments():
	...
	timingFilename = "dreal_time_rambus.csv"
...
def dRealSchmittExperiments():
	...
	timingFilename = "dReal_time_schmitt.csv"
	headers = ["Problem", "NumSolutions"]
	for i in range(numTrials):
		headers.append("Run_"+str(i))
	csvTimeData = [headers]
...
def dRealBenchmarkExperiments():
	...
	timingFilename = "dreal_time_benchmark.csv"
	headers = ["Problem"]
	for i in range(numTrials):
		headers.append("Run_"+str(i))
	csvTimeData = [headers]
...
def z3RambusExperiments():
	...
	timingFilename = "z3_time_rambus.csv"
	headers = ["Problem", "NumSolutions"]
	for i in range(numTrials):
		headers.append("Run_"+str(i))
	csvTimeData = [headers]
